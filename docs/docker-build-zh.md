...
services:
  ...
  frontend:
    ...
    depends_on:
      - backend
  # 可选：第三方中文优先刮削 Provider（ABS 生态）
  # 若需要自动拉取中文喜马/豆瓣元数据，建议额外部署下列镜像并在 backend 环境变量中声明地址：
  #   - shanyanwcx/abs-ximalaya:latest   (示例监听 http://abs-ximalaya:3000)
  #   - zqing90/audiobookshelf-provider-douban:latest (示例监听 http://abs-douban:3000)
  # 部署示例：
  #  abs_ximalaya:
  #    image: shanyanwcx/abs-ximalaya:latest
  #    container_name: abs_ximalaya
  #    restart: unless-stopped
  #  abs_douban:
  #    image: zqing90/audiobookshelf-provider-douban:latest
  #    container_name: abs_douban
  #    restart: unless-stopped
  # 使用时在 backend 环境变量追加：
  #   - ABS_XIMALAYA_PROVIDER_URL=http://abs-ximalaya:3000
  #   - ABS_DOUBAN_PROVIDER_URL=http://abs-douban:3000
```
