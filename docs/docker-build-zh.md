...
- 安装参数：`npm ci --no-optional`，已通过 `npm config set optional false` 关闭可选依赖，避免 @rollup/rollup-*-musl 安装失败。
...
- 安装参数：`npm ci --omit=dev --no-optional`，并提前 `npm config set optional false` 彻底禁用可选依赖。
- 暴露端口：3000
...
- Debian slim 基础镜像规避 musl 下的二进制可选包；`--no-optional` 和 `NPM_CONFIG_OPTIONAL=false` 双重保险。
